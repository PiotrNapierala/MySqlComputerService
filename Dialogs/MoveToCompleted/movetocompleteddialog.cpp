...
#include "movetocompleteddialog.h"
#include "ui_movetocompleteddialog.h"

MoveToCompletedDialog::MoveToCompletedDialog(Order *orderToMove, QWidget *parent) :
	QDialog(parent),
	ui(new Ui::MoveToCompletedDialog)
{
	ui->setupUi(this);
	...
	ui->doubleSpinBox_paid->setFocus();
}

MoveToCompletedDialog::~MoveToCompletedDialog()
{
	delete ui;
}

void MoveToCompletedDialog::LoadData(Order *order)
{
	...
	ui->doubleSpinBox_paid->setValue(order->valuation);
}

void MoveToCompletedDialog::on_pushButton_move_order_clicked()
{
	...
	if(connector.MoveOrder(currentOrder, connector.CCO_tb)) this->close();
}
